...
def carregar_dimensao_operadoras(engine):
    ...
    try:
        ...
        print("Falha ao ler UTF-8, tentando CP1252...")
        df = pd.read_csv(arq, sep=';', encoding='cp1252', dtype=str)
    except Exception as e:
        print(f"Tentando ISO-8859-1 devido a erro: {e}")
    ...
    cols_existentes = [c for c in df.columns if c in col_map]
...
def carregar_fato_despesas(engine):
    ...
    print(f"Linhas brutas carregadas: {len(df)}")
    
    df_agrupado = df.groupby(['cnpj_origem', 'ano', 'trimestre'])['valor'].sum().reset_index()
    
    df_nomes = df[['cnpj_origem', 'razao_social_snapshot']].drop_duplicates(subset=['cnpj_origem'])
    df_final = pd.merge(df_agrupado, df_nomes, on='cnpj_origem', how='left')
    
    print(f"Linhas consolidadas: {len(df_final)}")

    # Trade-off:
    # Utilizei Numeric(15, 2) para 'valor' em vez de Float.
    # Isso evita erros de precisão em cálculos financeiros.
    df_final.to_sql('fato_despesas', engine, if_exists='replace', index=False,
