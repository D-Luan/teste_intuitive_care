...
def buscar_zips_recursivamente(url_atual, profundidade_max=2, nivel_atual=0):
    """
    Navega na estrutura de diretórios da ANS para localizar 
    arquivos .zip.
    
    Args:
        profundidade_max: Limite de recursão para evitar 
        loops ou navegação infinita.
    Returns:
        Lista com URLs completas dos arquivos encontrados.
    """
    ...
    try:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if '.zip' in href.lower() and re.search(r'202\d', href):
                ...
                # A estrutura de pastas da ANS varia entre trimestres.
                # A busca recursiva garante que encontre o arquivo independente do subnível.
...
def buscar_links_demonstracoes(url_raiz):
    """
    Identifica os arquivos dos últimos 3 anos disponíveis 
    na API.
    """
...
def baixar_arquivo(url, pasta_destino):
    """
    Realiza o download do arquivo em chunks para não 
    sobrecarregar memória.
    """
    ...
    try:
        # Trade-off: 
        # Uso de stream=True para baixar em chunks.
        # Evita carregar arquivos grandes inteiros na RAM antes de salvar.
...
def main():
    ...
    print("Etapa: Demonstrações Contábeis")
